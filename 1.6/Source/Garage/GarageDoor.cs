...
namespace VanillaVehiclesExpanded
{
  ...
  public class GarageDoor : Building
  {
    ...
    protected override void Tick()
    {
      ...
      {
        var openGarage =
          ThingMaker.MakeThing(ThingDef.Named(def.defName + "Opened"), Stuff) as GarageDoor;
      ...
      {
        var closedGarage =
          ThingMaker.MakeThing(ThingDef.Named(def.defName.Replace("Opened", "")), Stuff) as
            GarageDoor;
    ...
    protected virtual IEnumerable<Gizmo> GetDoorGizmos()
    {
      ...
      else if (openDesignationOn && !opened)
      {
        var cancelButton = new Command_Action
        {
          ...
          action = delegate { base.Map.designationManager.RemoveDesignation(openDesignation); }
        };
      ...
      else if (closeDesignationOn && opened)
      {
        var cancelButton = new Command_Action
        {
          defaultLabel = "Cancel".Translate(),
          icon = ContentFinder<Texture2D>.Get("UI/Designators/Cancel"),
          action = delegate { base.Map.designationManager.RemoveDesignation(closeDesignation); }
        };
        yield return cancelButton;
      }
    }

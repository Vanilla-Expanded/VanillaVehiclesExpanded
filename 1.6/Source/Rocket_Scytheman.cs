...
namespace VanillaVehiclesExpanded
{
  ...
  public class Rocket_Scytheman : Projectile_Explosive
  {
    private Vector3 LookTowards =>
      new(this.destination.x - this.origin.x, this.def.Altitude, this.destination.z -
        this.origin.z +
    ...
    public void ThrowSmokeTrail(Vector3 loc, Map map, float angle, float size)
    {
      FleckCreationData dataStatic =
        FleckMaker.GetDataStatic(loc, map, VVE_DefOf.VVE_Scytheman_RocketSmoke, size);
    ...
    {
      FleckCreationData dataStatic =
        FleckMaker.GetDataStatic(loc, map, VVE_DefOf.VVE_Scytheman_RocketExhaust, size);
      dataStatic.velocityAngle = angle;
    ...
    {
      FleckCreationData dataStatic =
        FleckMaker.GetDataStatic(loc, map, FleckDefOf.DustPuffThick, scale);

...
namespace VanillaVehiclesExpanded
{
  ...
  {
    public static void Postfix(VehicleDef ___vehicleDef,
      ref IEnumerable<VehicleStatDrawEntry> __result)
    {
      if (___vehicleDef.GetModExtension<StatExtension>() != null)
      {
        ...
        if (extension.statToAdd != null)
        {
          ...
          {
            __result = __result.AddItem(new VehicleStatDrawEntry(StatCategoryDefOf.BasicsPawn,
              stat.Translate(), extension.statValues[extension.statToAdd.IndexOf(stat)].Translate(),
              extension.statDescriptions[extension.statToAdd.IndexOf(stat)].Translate()
            , 1));
          }
        }
      }

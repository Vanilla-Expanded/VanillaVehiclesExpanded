...
namespace VanillaVehiclesExpanded
{
  ...
  public static class Utils
  {
    ...
    public static bool IsDisabled(this ResearchProjectDef __instance, out ThingDef wreckNotRestored)
    {
      ...
      {
        if (restoredWreck.GetCompProperties<CompProperties_VehicleWreck>().researchDef ==
          __instance)
        {
          return false;
        }
      }
      foreach (var wreck in wreckDefs.Where(x =>
        GameComponent_VehicleUseTracker.Instance.restoredWrecks.Contains(x) is false))
      {

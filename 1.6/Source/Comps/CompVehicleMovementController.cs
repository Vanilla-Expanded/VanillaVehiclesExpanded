...
namespace VanillaVehiclesExpanded
{
    ...
    public class CompVehicleMovementController : VehicleComp
    {
        ...
        public override void CompTick()
        {
            ...
            if (!Vehicle.Spawned)
                return; //WorldPawns can tick.  If curPath was not cleared before ticking, exception will be thrown
            ...
            if (wasMoving && Vehicle.vehiclePather.curPath != null)
            {
                ...
                float decelerateInPctOfPath =
                  moveSpeed / (AccelerationRate * DecelerationMultiplier) / totalPathCost;
                ...
                {
                    if (curMovementMode != MovementMode.Decelerate 
                        && ShouldSpeedUp(remainingArrivalTicks))
                    {
                        SpeedUp(moveSpeed);
                    }
                    else
        ...
        private bool ShouldSpeedUp(float remainingArrivalTicks)
        {
            ...
            for (var i = 0; i < remainingArrivalTicks; i++)
            {
                ...
                var slowdownMultiplier = tempSpeed / (AccelerationRate * DecelerationMultiplier) /
                  (remainingArrivalTicks - i);
                if (tempSpeed <= 0)
                {
        ...
        public void Slowdown(float remainingArrivalTicks, float deceleratePct,
          bool stopImmediately = false)
        {
            remainingArrivalTicks += 30;
            float decelerationRate = AccelerationRate * DecelerationMultiplier;
            float newSpeed = currentSpeed - decelerationRate;
            float slowdownMultiplier = currentSpeed / (AccelerationRate * DecelerationMultiplier) /
            ...
            if (shouldApplyHandbrake)
            {
                ...
                isScreeching = true;
                screechingSustainer =
                  VVE_DefOf.VVE_TiresScreech.TrySpawnSustainer(SoundInfo.InMap(parent,
                    MaintenanceType.PerTick));
                handbrakeApplied = true;
                if (VanillaVehiclesExpandedSettings.handbrakeDealsDamage)
                {
                    Messages.Message("VVE_HandbrakeWarning".Translate(Vehicle.Named("VEHICLE")),
                      MessageTypeDefOf.NegativeHealthEvent);
                    float damageAmount = currentSpeed - (AccelerationRate * 10);
                    var components = Vehicle.statHandler.components.Where(x =>
                      x.props.tags != null && x.props.tags.Contains("Wheel"));
        ...
        public PathCostResult GetPathCost(bool ignorePassedCells)
        {
            ...
            result.cost += Vehicle.vehiclePather.nextCellCostTotal -
              Vehicle.vehiclePather.nextCellCostLeft;
        ...
        public override void PostSpawnSetup(bool respawningAfterLoad)
        {
            ...
            Vehicle.AddEvent(VehicleEventDefOf.Braking,
              TrySlowdown); //Hooks onto event trigger right before PathFollower is notified to stop
        }
        ...
        private void TrySlowdown()
        {
            if (Vehicle.vehiclePather.Moving && Vehicle.vehiclePather.curPath != null)
            {
                ...
                float decelerateInPctOfPath =
                  StatMoveSpeed / (AccelerationRate * decelerateMultiplier) / totalCost;
        ...
        public override string CompInspectStringExtra()
        {
            ...
            if (DebugSettings.godMode)
            {
                ...
                sb.AppendLine("--- VVE Movement Controller ---");
                sb.AppendLine($"Mode: {curMovementMode}");
                // "F2" formats the float to 2 decimal places.
                sb.AppendLine($"Speed: {currentSpeed.ToString("F2")} / {StatMoveSpeed.ToString("F2")}");
                // "P1" formats the float as a percentage with 1 decimal place.
                sb.AppendLine($"Path % Passed: {curPctOfPathPassed.ToString("P1")}");
